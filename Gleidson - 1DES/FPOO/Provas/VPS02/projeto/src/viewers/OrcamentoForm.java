...
import models.Orcamento;
	...
	public class OrcamentoForm extends JDialog implements ActionListener {
		...
		tableModel.addColumn("Recomendações");
		...
		private void buscar() {
			...
			if (isNumeric) {
				...
				if (OrcamentoProcess.orcamentos.contains(orcamento)) {
					...
					JOptionPane.showMessageDialog(this, "Orçamento/Produto não encontrado");
				}
			} else {
		...
		private void preencherTabela() {
			...
			for (Orcamento or : OrcamentoProcess.orcamentos) {
				...
				tableModel.addRow(new String[] { String.format("%d", or.getId()), or.getFornecedor(), or.getProduto(),String.format("%.2f", or.getPreco()), "Não Comprar"});
					}
				}
			}
